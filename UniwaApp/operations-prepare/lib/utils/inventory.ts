...
export function getStatusCounts(items: { status?: InventoryStatus | null }[] | InventoryStatus[]) {
  let unconfirmed = 0,
    confirmed = 0,
    notRequired = 0;
  items.forEach((item: any) => {
    const checkStatus =
      (item.status?.check_status ?? item.check_status) || getCode(INVENTORY_STATUS, 'UNCONFIRMED');
