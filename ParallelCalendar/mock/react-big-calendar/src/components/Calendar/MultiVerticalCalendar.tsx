...
const initialEvents: DomainEvent[] = [
  {
    ...
    start: new Date(2026, 0, 19, 7, 0),
    end: new Date(2026, 0, 19, 9, 0),
    realDate: new Date(2026, 0, 19),
    type: "do",
    description: "# Routine\n- Coffee\n- News",
  },
  {
...
interface MultiVerticalCalendarProps<
...
export const MultiVerticalCalendar = <
  TEvent extends object,
  TResource extends object,
>({
  startDate,
  ...
  getResourceTitle,
  calendarProps,
}: MultiVerticalCalendarProps<TEvent, TResource>) => {
  ...
  const resources = useMemo(() => {
    ...
    days.forEach((day) => {
      CalendarResources.forEach((resource) => {
        res.push({
          id: Adapter.toResourceId(day, getResourceId(resource)),
          title: getResourceTitle(resource),
          date: day,
        });
      });
      res.push({
        id: Adapter.toResourceId(day, "plan"),
        title: "PLAN",
        date: day,
      });
  ...
  const handleNavigate = useCallback(
    (newDate: Date) => setStartDate(newDate),
    [],
  );
  ...
  const onEventDrop = useCallback(
    (args: any) =>
      handleEventChange(args.event, args.start, args.end, args.resourceId),
    [handleEventChange],
  );
  const onEventResize = useCallback(
    (args: any) => handleEventChange(args.event, args.start, args.end),
    [handleEventChange],
  );
    ...
    >
      <DnDCalendar
        ...
        components={{
          ...
          toolbar: (props: any) => (
            ...
            >
              ...
              >
                {moment(startDate).format("YYYY年 M月 D日")} 〜{" "}
                {moment(startDate).add(6, "days").format("M月 D日")}
              </span>
            </div>
          ),
        }}
        ...
        eventPropGetter={(event: DomainEvent) => ({
          style: {
            backgroundColor:
              event.type === "plan" ? "var(--plan-bg)" : "var(--do-bg)",
            color:
              event.type === "plan" ? "var(--plan-text)" : "var(--do-text)",
