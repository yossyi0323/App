# テーブル定義書

## 概要
**ParallelCalendar（予実管理カレンダー）**
「タスク（Task）」を定義し、それを複数の「アロケーション（Allocation / 層）」に割り当てて管理する。
基本構造としては、**1つのタスクに対し、アロケーション（予定・実績など）ごとに1つの時間枠（TimeSlot）を持つ** 形となる。

## ER図 (Mermaid)

```mermaid
erDiagram
    users ||--o{ tasks : "owns"
    users ||--o{ time_slots : "owns"

    tasks ||--o{ time_slots : "has"

    users {
        uuid id PK
        text name
        text email
        timestamp created_at
        timestamp updated_at
        uuid created_by
        uuid updated_by
    }

    tasks {
        uuid id PK
        uuid user_id FK
        text title "タイトル"
        text description "詳細"
        timestamp created_at
        timestamp updated_at
        uuid created_by
        uuid updated_by
    }

    time_slots {
        uuid id PK
        uuid user_id FK
        uuid task_id FK
        text allocation "アロケーション"
        timestamp start_at "開始日時"
        timestamp end_at "終了日時"
        jsonb ext_data "拡張データ"
        timestamp created_at
        timestamp updated_at
        uuid created_by
        uuid updated_by
    }
```

## テーブル定義詳細

### 1. users (ユーザー)
アプリケーションの利用者。

| カラム名 | データ型 | 制約 | 説明 |
| :--- | :--- | :--- | :--- |
| `id` | UUID | PK | ユーザーID |
| `name` | TEXT | NOT NULL | 表示名 |
| `email` | TEXT | UNIQUE | メールアドレス |
| `created_at` | TIMESTAMPTZ | DEFAULT NOW() | 作成日時 |
| `updated_at` | TIMESTAMPTZ | DEFAULT NOW() | 更新日時 |
| `created_by` | UUID | FK -> users.id | 作成者 |
| `updated_by` | UUID | FK -> users.id | 更新者 |

### 2. tasks (タスク/定義)
「何をするか」というアセットそのもの。これがマスターデータとなる。
時間情報はここには持たず、あくまで「やること」の定義。

| カラム名 | データ型 | 制約 | 説明 |
| :--- | :--- | :--- | :--- |
| `id` | UUID | PK |  |
| `user_id` | UUID | FK -> users.id | 所有者 |
| `title` | VARCHAR | NOT NULL | タスク名（例：英語学習） |
| `description` | TEXT |  | 詳細メモ |
| `created_at` | TIMESTAMPTZ | DEFAULT NOW() | 作成日時 |
| `updated_at` | TIMESTAMPTZ | DEFAULT NOW() | 更新日時 |
| `created_by` | UUID | FK -> users.id | 作成者 |
| `updated_by` | UUID | FK -> users.id | 更新者 |



### 3. time_slots (時間帯/コマ)
タスクに紐付く具体的な時間枠。`allocation` カラムで「予定」か「実績」かといったアロケーションを管理する。
1つのタスクに複数のTimeSlotを紐付けられるため、**「予定なし・実績のみ」** や **「予定あり・実績なし」** といったケースも、単にレコードが存在するかどうかで表現可能。

| カラム名 | データ型 | 制約 | 説明 |
| :--- | :--- | :--- | :--- |
| `id` | UUID | PK |  |
| `user_id` | UUID | FK -> users.id | 所有者 |
| `task_id` | UUID | FK -> tasks.id | 実行するタスク |
| `allocation` | TEXT | NOT NULL | `PLAN`, `ACTUAL` 等のアロケーション |
| `start_at` | TIMESTAMPTZ | NOT NULL | 開始日時 |
| `end_at` | TIMESTAMPTZ | NOT NULL | 終了日時 |
| `ext_data` | JSONB |  | 拡張データ（フロントエンド用） |
| `created_at` | TIMESTAMPTZ | DEFAULT NOW() | 作成日時 |
| `updated_at` | TIMESTAMPTZ | DEFAULT NOW() | 更新日時 |
| `created_by` | UUID | FK -> users.id | 作成者 |
| `updated_by` | UUID | FK -> users.id | 更新者 |
