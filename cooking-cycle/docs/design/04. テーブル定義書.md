...
### Phase 2以降
- 必要に応じてカラム追加やテーブル追加を行う
- 既存データとの互換性を保つ

## 拡張性の考慮

### JSONカラムの活用（将来の拡張）
- 現在は使用していないが、将来の拡張に対応できるよう設計
- 例：レシピの`ingredients (JSON)`、ストックの`used_in_recipe_ids (JSON)`など

### 外部キー制約
- 必須の外部キーは制約を設定
- オプションの外部キー（`parent_recipe_id`、`created_from_recipe_id`）はNULLを許可
